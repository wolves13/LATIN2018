...
Without requiring the periodicity, one could simply design left and right-turn modules and concatenate their copies according to the sequence $P$. 
...
\begin{figure}[tb]
\includegraphics[width=0.9\linewidth]{pic/dragon_vol5.pdf}
\caption{
...
A turning module has its three bifurcator submodules direct further folding obtusely one after another when it receives O from the previous DFAO module (colored in blue in Fig.~\ref{fig:abst_dragon}) or acutely when it receives A (yellow), and folds into two L-shape conformations. 
