...
Such a ``hardcoding'' goes against the spirit of algorithmic self-assembly, and an oritatami system that folds into the infinite Heighway dragon, if any, could not take this approach in order to be describable by a finite mean. 
...
The half, or more precisely, its transcript, consists of three subsequences for the counter, DFAO, and turning modules. 
It folds as abstracted in Fig.~\ref{fig:abst_dragon} into one line segment plus one turn of the dragon and has its three modules accomplish the following tasks, respectively: 
...
We shall explain how modules read something in Sect.~\ref{subsubsec:module_counter}. 
